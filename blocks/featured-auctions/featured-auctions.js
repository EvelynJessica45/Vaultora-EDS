...
/**
 * Decorates the featured-auctions block by combining CMS headers with live cloud data arrays
 * @param {Element} block The block element
 */
export default async function decorate(block) {
  const rows = [...block.children];
  ...
  // 1. Maintain editorial layout configurations from document table
  const titleText = rows[0]?.innerText?.trim();
  const descText = rows[1]?.innerText?.trim();
  const placeholderText = rows[2]?.innerText?.trim() || 'Search auctions...';

  // Build editorial header container
  const headerContainer = document.createElement('div');
  headerContainer.className = 'featured-auctions-header-editorial';
  ...
  const filtersContainer = document.createElement('div');
  filtersContainer.className = 'quick-chips-track featured-auctions-quick-chips';
  ...
  // 2. Fetch active products data completely from cloud arrays
  const liveProducts = getProducts() || [];
  
  // Filter to display only active auction items on the premium layout track grid
  const activeAuctions = liveProducts.filter(product => product && product.auctionStatus === 'active');
  ...
  // Generate responsive high-density cards using live object properties
  activeAuctions.forEach((product, idx) => {
    const card = document.createElement('div');
    card.className = 'featured-auctions-editorial-card';

    // Parse image location array properties safely
    const currentPriceNum = parseFloat(product.currentBid || product.startingBid || product.price) || 0;
    
    card.dataset.price = currentPriceNum;
    ...
    if (imgSrc) {
      const optimizedPicture = createOptimizedPicture(imgSrc, product.title || 'Auction Item', false);
      imageWrap.append(optimizedPicture);
    } else {
      // Clean graphic placeholder fallback if asset references are missing
      imageWrap.innerHTML = `<div style="width:100%; height:100%; background:#f4f3ee;"></div>`;
    }
    ...
    const cardTitle = document.createElement('h3');
    cardTitle.className = 'card-editorial-title';
    cardTitle.textContent = product.title || 'Untitled Masterpiece';

    const cardAuthor = document.createElement('p');
    cardAuthor.className = 'card-editorial-author';
    cardAuthor.textContent = product.author || product.seller || 'Conscious Artist';
    ...
    const cardPrice = document.createElement('span');
    cardPrice.className = 'card-editorial-price';
    cardPrice.textContent = `₹${currentPriceNum.toLocaleString('en-IN', { minimumFractionDigits: 2 })}`;
    ...
    // Smooth link routing pass to direct profile parameters to bid detail views
    bidBtn.onclick = () => {
      window.location.href = `/my-bid-details?id=${encodeURIComponent(product.id)}`;
    };
  ...
  // ── CLIENT CONTROL SORT OPERATION FILTERS ──
  filtersContainer.addEventListener('click', (e) => {
    const clickedChip = e.target.closest('.filter-chip');
    if (!clickedChip) return;
    ...
    if (mode === 'high-bids') {
      cardElementsArray.sort((a, b) => parseFloat(b.dataset.price) - parseFloat(a.dataset.price));
    } else if (mode === 'low-price') {
      cardElementsArray.sort((a, b) => parseFloat(a.dataset.price) - parseFloat(b.dataset.price));
    } else {
      cardElementsArray.sort((a, b) => a.dataset.index - b.dataset.index);
    }
  ...
  // ── INLINE CLIENT SEARCH EVALUATION FUNCTION ──
  searchInput.addEventListener('input', () => {
    const query = searchInput.value.toLowerCase().trim();
    
    cardElementsArray.forEach((card, idx) => {
      const productData = activeAuctions[idx];
      const titleMatch = productData.title?.toLowerCase().includes(query);
      const authorMatch = productData.author?.toLowerCase().includes(query) || productData.seller?.toLowerCase().includes(query);

      if (!query || titleMatch || authorMatch) {
  ...
  // Render the components into the block canvas container
  block.textContent = '';
  block.append(headerContainer, cardsGrid);
}