...
(function () {
  ...
  function initHeaderBlock() {
    ...
    // 1. Text Logo Rendering matching image_749880
    logoCell.innerHTML = 'Vault<span>ora</span>';
    logoCell.onclick = (e) => { e.preventDefault(); window.location.href = 'index.html'; };

    // 2. Responsive Burger Trigger
    ...
    if (linkParagraphs.length > 0) {
      linkParagraphs.forEach(p => {
        const cleanText = p.textContent.replace(/[•·]/g, '').trim();
        if (cleanText) linksHTML += `<a href="#" data-nav="${cleanText}">${cleanText}</a>`;
      });
      linksCell.innerHTML = linksHTML;
    }

    if (!linksCell.innerHTML || linksCell.querySelectorAll('a').length < 3) {
      linksCell.innerHTML = `
        <a href="#" data-nav="Buy">Buy</a>
        <a href="#" data-nav="Sell">Sell</a>
        <a href="#" data-nav="Auctions">Auctions</a>
        <a href="#" data-nav="Categories">Categories</a>
        <a href="#" data-nav="How it Works">How it Works</a>
      `;
    }
    ...
    const mapPathToNav = {
      ...
      'dashboard.html': 'Auctions', 'mybids.html': 'My Bids', 'mylisted.html': 'My Listing'
    };
    ...
    const searchLink = `<a href="#" class="header-icon-btn" data-nav="search" title="Search">${searchIconSVG}</a>`;
    ...
    if (session) {
      ...
      actionsCell.innerHTML = `${heartLink} ${searchLink} ${bagLink}`;
      ...
      actionsCell.innerHTML = `
        <button class="btn-signup-premium" id="headerSignupBtn">Sell an Item</button>
        ${heartLink}
        ${searchLink}
        ${bagLink}
      `;
